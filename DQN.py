...
class Agent:
    ...
    def compute_loss_dqn(self, states, actions, rewards, next_states, done_flags, gamma = 0.99):
        pred_qs = self.network(states) #predictions calculated using multilayer perceptron
        tar_qs = self.target_net(next_states) #targets computed using a target network
        
        row_indices = tf.range(actions.shape[0])
        indices = tf.transpose([row_indices, actions])

        #print(f'indices : {indices}')
        #predicted qvals of actions taken
        pred_qvals = tf.gather_nd(pred_qs, indices) #used to isolate Q-values of actions taken in the given states
        #max qval of next states
        tar_qvals = tf.reduce_max(tar_qs, axis = -1)

        #print(f'pred qvals : {pred_qvals} targets : {tar_qvals}')
        tar_qvals_calc = rewards + gamma*(1-done_flags)*tar_qvals

        loss = tf.keras.losses.MSE(pred_qvals,tar_qvals_calc)
        return loss

    def dqn(self, stop_at_rewards=220):
        ...
        for episode in range(self.num_episodes):
            ...
            for step in range(self.num_steps):
                ...
                with tf.GradientTape() as tape:
                    loss = self.compute_loss_dqn(s,a,r,s_,d)
