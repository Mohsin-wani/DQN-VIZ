...
class Dueling_Qnet(tf.keras.Model):
    def __init__(self, n_actions, num_layers = 3, dimensions = [256,128,64], activations = ['relu','relu','relu'], final_activation = 'linear'):
        super(Dueling_Qnet,self).__init__()

        self.mlp_layers = []
        for i in range(num_layers-1):
            self.mlp_layers.append(tf.keras.layers.Dense(dimensions[i], activation= activations[i]))
        #self.mlp_layers.append(tf.keras.layers.Dense(n_actions, activation= activations[i+1]))
    ...
    def call(self, state):
        ...
        ## The net effect of the steps if Q(s,a) = V(s) + [A(s,a) - 1/n_actions * SUM(A(s,a`))] the average forms a baseline
        ## against which the rest of the advantages are computed. Summing up with the Value give the Q(s,a).
        #########################################
        qval = tf.math.subtract(v,avg_adv)
        qval = tf.math.add(qval, adv)
        #########################################
        return qval  
    
class Dueling_Cnn_Qnet(tf.keras.Model):
    def __init__(self, n_actions, num_cnn_layers = 3, channels = [32,64,32], activations = ['relu','relu','relu'], final_activations = 'linear', filter_size = [(3,3),(3,3),(3,3)], strides= [2,2,2]):
        super(Dueling_Cnn_Qnet,self).__init__()
        self.cnn_layers = []
...
class Agent:
    def __init__(self, env, hyp_params):
        ...
        self.network = Dueling_Qnet(self.n_actions)
        self.target_net = Dueling_Qnet(self.n_actions)
    ...
    #DQN algorithms using bellman update to learn
            ...
            if self.hyp_params.record == True:
                if episode >= self.hyp_params.start_eps_rec:
                    if episode % self.hyp_params.frequency_rec == 0 and episode != 0:                                   
                        self.util.record_video(self.env,self.network, 2, episode, self.hyp_params.req_reward, path = self.hyp_params.path)
